...
$posts = $db->createCommand("SELECT availability.id, FROM_UNIXTIME(start_time, '%d.%m.%Y') as 'Start Datum',  FROM_UNIXTIME(start_time, '%d.%m.%Y %h:%i') as Start, FROM_UNIXTIME(end_time, '%d.%m.%Y %h:%i') as Ende, (duration_sec) as 'Dauer(sek)', (duration_sec/60) as 'Dauer(min)', fault_code.name as Störungsbeschreibung, fault_code_group.name as Störungsgruppe, work_centre.name as Arbeitstation FROM `availability`
