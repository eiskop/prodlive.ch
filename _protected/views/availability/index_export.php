...
$posts = $db->createCommand("SELECT availability.id, CONCAT(YEAR(FROM_UNIXTIME(start_time, '%Y-%m-%d')), '-',  LPAD(WEEK(FROM_UNIXTIME(start_time, '%Y-%m-%d'), 3), 2, '0')) as 'J-W', FROM_UNIXTIME(start_time, '%d.%m.%Y') as 'Start Datum',  FROM_UNIXTIME(start_time, '%d.%m.%Y %h:%i') as Start, FROM_UNIXTIME(end_time, '%d.%m.%Y %h:%i') as Ende, (duration_sec) as 'Dauer(sek)', format(duration_sec/60, 0) as 'Dauer(min)', Format(duration_sec/60/60, 2) as 'Dauer(h)', fault_code.name as Störungsbeschreibung, comment as Kommentar, fault_code_group.name as Störungsgruppe, work_centre.name as Arbeitstation FROM `availability`
